...
public class GameManager : MonoBehaviour
{
    ...
    [Header("Time Limited")]
    [Tooltip("Time limit in which this level can be completed")]
    [SerializeField] private float timeLimit = 45.0f;
    public bool isTimeLimited = true;
    private float gameManagerCurrentTime = 0.0f;
    // Static getter/setter for current time(for convenience)
    public static float currentTime
    {
        get
        {
            return instance.gameManagerCurrentTime;
        }
        set
        {
            instance.gameManagerCurrentTime = value;
        }
    }
    ...
    private void Update()
    {
        if (isTimeLimited && !gameIsOver)
        {
            if (!levelCleared)
            {
                currentTime -= Time.deltaTime;
                if (currentTime <= 0.0 && !gameIsOver)
                {
                    //update timer text
                    GameOver();
                }
